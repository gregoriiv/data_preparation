...
def network_collection(conf=None,database=None):
    ...
    os.chdir(work_dir)
    password_cmdline(f'osm2pgsql -d {dbname} -H {host} -U {username} --port {port} --hstore -E 4326 -W -r .osm -c src/data/input/merged_osm.osm', password=password)
    #subprocess.run(f'osm2pgsql -d {dbname} -H {host} -U {username} --port {port} --hstore -E 4326 -W -r .osm -c src/data/input/merged_osm.osm', shell=True, check=True)
    os.chdir('src/data/input')
