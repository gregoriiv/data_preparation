...
AND w.class_id::text NOT IN (SELECT UNNEST(ARRAY{var["variable_container"]["excluded_class_id_walking"]}::text[]))
AND w.class_id::text NOT IN (SELECT UNNEST(ARRAY{var["variable_container"]["excluded_class_id_cycling"]}::text[]))
AND (
(w.foot NOT IN (SELECT UNNEST(ARRAY{var["variable_container"]["categories_no_foot"]})) OR foot IS NULL)
OR
(w.bicycle NOT IN (SELECT UNNEST(ARRAY{var["variable_container"]["categories_no_bicycle"]})) OR bicycle IS NULL))
...
WITH variables AS
(
    SELECT '{json.dumps(var["variable_container"]["wheelchair"])}'::jsonb  AS wheelchair,
    ARRAY{var["variable_container"]["excluded_class_id_walking"]}::text[] AS excluded_class_id_walking,
    ARRAY{var["variable_container"]["categories_no_foot"]} AS categories_no_foot,
    ARRAY{var["variable_container"]["categories_sidewalk_no_foot"]} AS categories_sidewalk_no_foot
) 
...
WITH RECURSIVE ways_no_islands AS (
	...
	(SELECT id,geom
	FROM ways w
	WHERE w.class_id::text NOT IN (SELECT UNNEST(ARRAY{var["variable_container"]["excluded_class_id_walking"]}::text[]))
	AND w.class_id::text NOT IN (SELECT UNNEST(ARRAY{var["variable_container"]["excluded_class_id_cycling"]}::text[]))
	AND (
	(w.foot NOT IN (SELECT UNNEST(ARRAY{var["variable_container"]["categories_no_foot"]})) OR foot IS NULL)
	OR
	...
	AND w.class_id::text NOT IN (SELECT UNNEST(ARRAY{var["variable_container"]["excluded_class_id_walking"]}::text[]))
	AND w.class_id::text NOT IN (SELECT UNNEST(ARRAY{var["variable_container"]["excluded_class_id_cycling"]}::text[]))
	AND (w.foot NOT IN (SELECT UNNEST(ARRAY{var["variable_container"]["categories_no_foot"]})) OR foot IS NULL)
) 
SELECT w.id  
FROM (
	SELECT w.id
